...
/** @deprecated use {@link slimeknights.tconstruct.library.modifiers.modules.armor.ArmorWalkRadiusModule} */
...
public abstract class AbstractWalkerModifier extends NoLevelsModifier implements ArmorWalkModifierHook {
  ...
  public void onWalk(IToolStackView tool, ModifierEntry modifier, LivingEntity living, BlockPos prevPos, BlockPos newPos) {
    ...
    if (living.onGround() && !tool.isBroken() && !level.isClientSide) {
      float trueRadius = Math.min(16, getRadius(tool, modifier.getLevel()));
      ...
      for (BlockPos pos : BlockPos.betweenClosed(center.offset(-radius, 0, -radius), center.offset(radius, 0, radius))) {
        if (pos.closerToCenterThan(living.position(), trueRadius)) {
          walkOn(tool, modifier.getLevel(), living, level, pos, mutable);
          if (tool.isBroken()) {
            break;
          }
        }
